...
MODULE_ROOT ?= fry.org/qft/jumble
...
coverage: ## Generates the total code coverage of the project
	...
	$(Q)for j in $$(go list $(TOP_LEVEL)/... | grep -v '/vendor/' | grep -v '/ext/'); do go test -covermode=count -coverprofile=$(COVERAGE_DIR)/$$(basename $$j).out  $$j > /dev/null 2>&1; done
...
package: ## Create a docker image of the project
	...
	$(Q)docker build $(BUILD_CACHE) \
		...
		--label 'org.label-schema.vendor=QuantumFry Solutions' \
...
unit_test: ## Run all available unit tests
	go test -v $(shell go list $(TOP_LEVEL)/... | grep -v /vendor/)
